    ...
    Os recursos computacionais necessários são divididos entre:
    recursos disponíveis para desenvolvimento e recursos
    necessários para utilização descritos nas Seções
    \ref{sec:recursosdeselvolvimento} e \ref{sec:recursosutilizacao}
    respectivamente.

    \UTFPRsection[sec:recursosdeselvolvimento]{RECURSOS DISPONÍVEIS PARA DESENVOLVIMENTO}
    ...
    \UTFPRsubsection{Recursos de \emph{Software}}
        ...
        \begin{enumerate}
            ...
            Necessário para visualização e alteração
            ...
            \item[e)] \emph{Visual Paradigm versão 5.2}:
            Utilizado para construção dos diagramas UML;

            \item[f)] \emph{Mozilla Firefox versão 1.5.0.5}:
            Navegador compatível com as especificações HTML 4.01,
            ...
            programação utilizada neste trabalho;
            ...
            para criação dos grafos.

        \end{enumerate}

        \end{enumerate}

    \UTFPRsection[sec:recursosutilizacao]{RECURSOS NECESSÁRIOS PARA UTILIZAÇÃO}
    ...
    \UTFPRsubsection{Usuário}
        ...
        especificações HTML 4.01, CSS 2 com suporte a
        JavaScript.

    \UTFPRsubsection{\emph{Website} Hospedeiro}

        O sistema elaborado, poderá ser implantado em \emph{websites} comuns
