...
\subsection{O que é um DataAdapter?}

\begin{frame}
    \frametitle{O que é um DataAdapter?}
    \begin{block}{Definição}
        O \emph{DataAdapter} é a classe responsável por fazer a interação
        entre a base de dados e o \emph{DataSet}.
    \end{block}
    \begin{itemize}
        \item Capaz de executar os quatro comandos básicos de um banco de dados (\emph{Insert, Update, Delete, Select});
        \item Para extrairmos dados da base de dados e preenchermos o \emph{DataSet} utilizamos a classe \emph{DataAdapter};
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{block}{Instruções}
        ...
        \emph{query} e uma conexão para a extração dos dados.
    \end{block}
    \texttt{\texttt{\lstinputlisting[language=C, label=mdiFilha,
